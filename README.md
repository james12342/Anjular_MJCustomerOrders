# Angular MJ Customer Orders with TypeScript

The goal of this MJ Customer Orders app is to provide
...
1. Go to http://localhost:8089 in your browser 
...
1. Go to http://localhost:By in your browser 
...
skaffold init -k '.k8s/*.yml' \
  -a '{"builder":"Docker","payload":{"path":".docker/nginx.dev.dockerfile"},"image":"nginx-angular-MJ Customer Orders"}' \
  -a '{"builder":"Docker","payload":{"path":".docker/node.dockerfile"},"image":"node-service-MJ Customer Orders"}'
```
...
3. Docker Stacks --> Docker Desktop --> Kubernetes
    ...
    ```
    docker stack deploy -c docker-compose.yml angular-MJ Customer Orders
    docker stack ls
    docker stack rm angular-MJ Customer Orders
    ```
